...
### 🧮 Strategy creation (greenfield)
| File | Role | What it does each cycle |
|---|---|---|
| [01-quant-mathematician.md](01-quant-mathematician.md) | Quant Mathematician | Renaissance-style first-principles strategy ideation + backtest. |
| [02-mean-reversion-engineer.md](02-mean-reversion-engineer.md) | Mean Reversion Engineer | Engineered reversion systems, no RSI/BB soup. |
...
| [04-volatility-strategist.md](04-volatility-strategist.md) | Volatility Strategist | Vol compression/expansion, vol-of-vol, realized-vs-implied edges. |
| [05-statistical-arbitrage-researcher.md](05-statistical-arbitrage-researcher.md) | Stat Arb Researcher | Cointegration, pairs, spread mean reversion, basket trades. |

### 🔧 Optimization
| File | Role | What it does each cycle |
|---|---|---|
| [06-strategy-optimizer.md](06-strategy-optimizer.md) | Strategy Optimizer | Searches Trader Dev, forks winners, improves one variable at a time. |
...
| File | Role | What it does each cycle |
|---|---|---|
| [11-funding-rate-strategist.md](11-funding-rate-strategist.md) | Funding Rate Strategist | Edges from perp funding rates, basis, OI, open interest decay. |
| [12-liquidity-sweep-hunter.md](12-liquidity-sweep-hunter.md) | Liquidity Sweep Hunter | Stop-hunt and sweep-and-reverse behaviour around liquidity pools. |

### 🧪 Experimental
| File | Role | What it does each cycle |
|---|---|---|
| [13-moon-phase-strategist.md](13-moon-phase-strategist.md) | Moon Phase Strategist | Tests lunar/astrology hypotheses honestly. Most fail. The point is to prove it. |
| [14-sentiment-strategist.md](14-sentiment-strategist.md) | Sentiment Strategist | News, social, funding sentiment as crypto-specific edge. |
...
1. **Each fire is independent.** Do not assume context from the previous cycle exists.
2. **One productive cycle per fire.** Pick one strategy, do one job, write one report.
3. **Persist to Trader Dev, not to chat history.** Use `create_strategy` / `update_strategy` so work survives across cycles.
4. **Fail fast.** If the cycle hits a dead end (no candidates, MCP errors, weak edge), report and stop — don't pretend.
5. **Honest verdicts only.** Reject / Watchlist / Incubate / Candidate / Production candidate. No hype.
6. **No live trading from a loop.** Loops are research and backtesting only. Live execution always requires human review.
